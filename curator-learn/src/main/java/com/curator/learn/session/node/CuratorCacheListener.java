...
public class CuratorCacheListener {
			...
			client.create().creatingParentsIfNeeded()
					.withMode(CreateMode.EPHEMERAL).inBackground(new BackgroundCallback() {
						public void processResult(CuratorFramework client, CuratorEvent event)
							...
							latch.countDown();
							client.setData().forPath("/zk-test-ephemeral-","hello".getBytes());
						}
					},tp)
					.forPath("/zk-test-ephemeral-", "init".getBytes());
			
			 @SuppressWarnings("resource")
			final NodeCache cache = new NodeCache(client,"/zk-test-ephemeral-",false);
			 cache.start(true);
